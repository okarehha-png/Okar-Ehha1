...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      let errorMessage = "Invalid credentials. Please verify your access details.";
      if (err.code === 'auth/operation-not-allowed') {
        ...
        errorMessage = "Account not found or invalid password. Have you created the user in Firebase yet?";
      } else if (err.message) {
    ...
    <div className="min-h-screen bg-[#F5F5F7] flex items-center justify-center p-6">
      <div className="bg-white p-10 md:p-12 rounded-[24px] shadow-sm w-full max-w-[440px] border border-gray-200/60">
        ...
        <form onSubmit={handleEmailLogin} className="space-y-6">
          ...
          >
            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Authenticate with Email"}
          </button>
        </form>

        <div className="mt-6 flex items-center justify-between">
          <hr className="w-full border-gray-200" />
          <span className="p-2 text-gray-400 text-xs font-bold uppercase tracking-wider">OR</span>
          <hr className="w-full border-gray-200" />
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-white border-2 border-gray-200 text-gray-900 px-6 py-4 rounded-xl font-bold text-base hover:border-gray-300 hover:bg-gray-50 transition-colors mt-6 flex items-center justify-center gap-3 disabled:opacity-50"
        >
